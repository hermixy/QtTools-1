...
namespace varalgo
{
	...
	inline InputIterator find_if(InputIterator first, InputIterator last, Pred && pred)
	{
		auto alg = [&first, &last](auto && pred)
		{
			return std::find_if(first, last, std::forward<decltype(pred)>(pred));
		};
	...
	inline auto find_if(const SinglePassRange & rng, Pred && pred)
	{
		return varalgo::find_if(boost::begin(rng), boost::end(rng), std::forward<Pred>(pred));
	}

	template <class SinglePassRange, class Pred>
	inline auto find_if(SinglePassRange & rng, Pred && pred)
	{
		return varalgo::find_if(boost::begin(rng), boost::end(rng), std::forward<Pred>(pred));
	}
		...
		-> typename boost::range_return<const SinglePassRange, re>::type
	{
		return boost::range_return<const SinglePassRange, re>::pack(
			varalgo::find_if(boost::begin(rng), boost::end(rng), std::forward<Pred>(pred)),
			rng);
	}
