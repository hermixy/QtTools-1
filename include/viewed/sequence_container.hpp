...
{
	/*
	/// container_traits describes store type, update action, etc for sequence_container.
	/// see also ptr_vector_traits and poly_ptr_vector_traits as default implementations
	struct container_traits
	{
		...
		/// obtains reference(should be reference, not a value_type) from internal_value_type (from main_store_type)
		...
		/// Constructs element in internal_value_type form from some argument.
		/// Typically it will be value_type ref, but can be pointer for example,
		/// if you want to clearly state polymorphic nature
		static internal_value_type make_internal(const auto & something) { return val; }
	};
	*/

	/// default sequence_container traits. Elements stored in vector<std::unique_ptr<Type>>
	template <class Type>
	struct ptr_vector_traits
	{
		...
		static internal_value_type make_internal(Type && val)      { return std::make_unique<Type>(std::move(val)); }
		static internal_value_type make_internal(const Type & val) { return std::make_unique<Type>(val); }
	...
	/// same as ptr_vector_traits, but elements are inserted from pointers
	template <class Type>
	struct poly_ptr_vector_traits
	{
		using internal_value_type = std::unique_ptr<Type>;
		using main_store_type     = std::vector<internal_value_type>;
		using signal_store_type   = std::vector<const Type *>;
		...
		static main_store_type make_store()                  { return {}; }
		static internal_value_type make_internal(Type * val) { return internal_value_type(val); }
	...
	*/
	...
	/// Iterators are read-only. use assign/append/erase add/remove data
	/// 
	...
	template <
		class Type,
		class Traits = ptr_vector_traits<Type>,
	...
	protected:
		...
		class iterator_adaptor : public boost::iterator_adaptor<iterator_adaptor<iterator_base>, iterator_base, value_type>
		{
			friend boost::iterator_core_access;
			using base_type = boost::iterator_adaptor<iterator_adaptor<iterator_base>, iterator_base, value_type>;
	...
	public:
		using const_iterator = iterator_adaptor<typename main_store_type::const_iterator>;
		using const_reverse_iterator = iterator_adaptor<typename main_store_type::const_reverse_iterator>;

		using iterator = const_iterator;
		using reverse_iterator = const_reverse_iterator;
		//using iterator = iterator_adaptor<typename main_store_type::iterator>;
		//using reverse_iterator = iterator_adaptor<typename main_store_type::reverse_iterator>;
